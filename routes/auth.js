...
router.get('/login_empresa', (req, res) => {
  res.render('auth/login_empresa', { message: null });
});
...
router.post('/login_empresa', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign(
      { userId: empresa.id, role: 'empresa' },
...
router.get('/forgot-password-empresa', (req, res) => {
  res.render('auth/forgot-password-empresa', { message: null });
});
...
router.post('/forgot-password-empresa', async (req, res) => {
  ...
  try {
    ...
    if (!empresa) {
      return res.render('auth/forgot-password-empresa', {
    ...
    // Configurar e-mail
    const resetUrl = `${process.env.BASE_URL}/auth/reset-password-empresa/${resetToken}`;
    const mailOptions = {
      from: process.env.EMAIL_USER,
      to: email,
    ...
    res.render('auth/forgot-password-empresa', {
      message: 'Um e-mail foi enviado com as instruções para recuperar sua senha.'
    });

  } catch (error) {
    console.error('Erro ao processar recuperação de senha:', error);
    res.render('auth/forgot-password-empresa', {
      message: 'Erro ao processar a recuperação de senha. Tente novamente.'
    });
  }
});

// Rota para exibir o formulário de redefinição de senha
router.get('/reset-password-empresa/:token', async (req, res) => {
  try {
    ...
    res.render('auth/reset-password-empresa', {
...
router.post('/reset-password-empresa/:token', async (req, res) => {
  ...
  try {
    ...
    if (!empresa) {
      return res.render('auth/reset-password-empresa', {
        token,
        message: 'Token inválido ou expirado.'
      });
    }
    ...
    res.redirect('/auth/login_empresa?message=Senha atualizada com sucesso');
  } catch (error) {
...
// Rota para exibir o formulário de recuperação de senha do candidato
router.get('/forgot-password-candidato', (req, res) => {
  res.render('auth/forgot-password-candidato', { message: null });
});

// Rota para processar a solicitação de recuperação de senha do candidato
router.post('/forgot-password-candidato', async (req, res) => {
  ...
  try {
    ...
    if (!candidato) {
      return res.render('auth/forgot-password-candidato', {
        message: 'E-mail não encontrado.'
      });
    }
    ...
    const resetUrl = `${process.env.BASE_URL}/auth/reset-password-candidato/${resetToken}`;
    const mailOptions = {
      from: process.env.EMAIL_USER,
      to: email,
      subject: 'Recuperação de Senha',
    ...
    res.render('auth/forgot-password-candidato', {
      message: 'Um e-mail foi enviado com as instruções para recuperar sua senha.'
    });
...
router.get('/reset-password-candidato/:token', async (req, res) => {
  try {
    ...
    res.render('auth/reset-password-candidato', {
      token: req.params.token,
      message: null
    });
  } catch (error) {
...
router.post('/reset-password-candidato/:token', async (req, res) => {
  const { password } = req.body;
  const { token } = req.params;
  ...
  try {
    ...
    if (!candidato) {
      return res.render('auth/reset-password-candidato', {
        token,
        message: 'Token inválido ou expirado.'
      });
    }
    ...
    res.redirect('/auth/login_candidato?message=Senha atualizada com sucesso');
  } catch (error) {
    console.error('Erro ao redefinir senha:', error);
    res.render('auth/reset-password-candidato', {
      token,
